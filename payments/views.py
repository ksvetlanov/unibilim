...
class InitiatePaymentView(APIView):
    def post(self, request, format=None):
        ...
        # инициируем платеж и получаем данные для запроса
        payment_data = self.initiate_payment(total_amount)

        # формирование описания платежа
        description_lines = []
        for slot in time_slots:
            professor_id = slot.get('professor_id')
            time_slot = slot.get('time_slot')
            description_lines.append(f'Payment for {service}({time_slot})')
    ...
    def initiate_payment(self, amount):
        ...
        return request  # Вы можете вернуть более полезные данные здесь, например response.json(), если это необходимо
